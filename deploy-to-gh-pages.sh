...
# Deploys game files to gh-pages branch for GitHub Pages hosting
...
show_usage() {
    ...
    echo "  $0 -m 'Update game UI'   # Deploy with custom message"
...
check_prerequisites() {
    ...
    if [[ ! -f "pokerv2/client/index.html" ]] || [[ ! -d "pokerv2/client/src" ]] || [[ ! -d "pokerv2/client/assets" ]]; then
        log_error "Essential game files not found (pokerv2/client/index.html, pokerv2/client/src/, pokerv2/client/assets/)"
...
# Function to deploy game files
deploy_files() {
    log "Deploying game files..."
    ...
    # Check if we're in the gh-pages branch (which won't have the game files)
    if [[ ! -f "pokerv2/client/index.html" ]]; then
        # We're in gh-pages branch, need to copy from the original branch
        log "Game files not found in current branch, copying from original branch..."
        
        # Temporarily checkout the original branch to copy files
        git checkout "$CURRENT_BRANCH" -- pokerv2/client/
        
        # Copy files from pokerv2/client to root of gh-pages
        if [[ -d "pokerv2/client" ]]; then
            cp -r pokerv2/client/* .
            rm -rf pokerv2/
            log_success "Copied game files from pokerv2/client to root"
        else
            log_error "Could not copy files from pokerv2/client"
            exit 1
        fi
    else
        # We're already in the original branch, copy files to root
        cp -r pokerv2/client/* .
        rm -rf pokerv2/
        log_success "Copied game files from pokerv2/client to root"
    fi
