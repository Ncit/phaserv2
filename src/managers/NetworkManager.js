...
export class NetworkManager {
    ...
    getServerUrl() {
        ...
        if (isDevelopment) {
            ...
            console.log('NetworkManager: Using local server for development:', localServerUrl);
            return localServerUrl;
        } else {
            // Use the remote server for production
            // The /poker path is handled by Traefik routing
            const remoteServerUrl = `${window.location.protocol}//${window.location.host}`;
            console.log('NetworkManager: Using remote server for production:', remoteServerUrl);
                ...
                import('https://cdn.socket.io/4.7.2/socket.io.esm.min.js')
                    .then(({ io }) => {
                        ...
                        const socketOptions = {
                            ...
                            path: isDevelopment ? '/socket.io' : '/pokerserver/socket.io',
                            forceNew: true,
                            autoConnect: true
                        };
